function rowAndMaximumOnes(mat) {
    let maxOnes = 0; // Initialize the maximum number of ones to 0
    let rowIndex = 0; // Initialize the row index to 0
    // Iterate through each row
    for (let i = 0; i < mat.length; i++) {
        // Count the number of 1's in the current row
        let onesCount = mat[i].reduce((acc, num) => acc + num, 0);
