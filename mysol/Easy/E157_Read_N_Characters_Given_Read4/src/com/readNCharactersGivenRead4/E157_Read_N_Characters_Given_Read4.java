package com.readNCharactersGivenRead4;
import java.util.*;
...
Note: buf4[] is destination not source, the results from read4 will 
be copied to buf4[]
...
a total of 3 characters from the file, so return 3. Note that "abc" is the 
file's content, not buf. buf is the destination buffer that you will have to 
...
public class E157_Read_N_Characters_Given_Read4 {
	...
	public static void main(String[] args) {
		...
		int output = read(input.toCharArray(), 5);
        System.out.println("input: " + (input) + "\noutput: " + (output));
	}
	
	/*
    solution:
    从read4读出来的字符存到buff里 总共要读n个字符或者直到文件读完 
    这个时候就要退出循环 返回实际总共读了多少字符
    */
